...
import ConecaoBD #Classe que faz a conexão com o banco de dados
db_connection = ConecaoBD.conectar()
con = db_connection.cursor()

def inserir(nome, telefone, endereco, dataDeNascimento):
    try:
        sql ="inserir into pessoa(codigo, nome, telefone, endereco, dataDeNascimento) values('', '{}', '{}', '{}', '{}')".format(nome, telefone, endereco, tratarData(dataDeNascimento))
        con.execute(sql)
...
def consultar():
    try:
        ...
        for(codigo, nome, telefone, endereco, dataDeNascimento) in con:
            print('Códico: {}, Nome: {}, Telefone: {}, Endereco: {}, Data de Nascimento: {}'.format(codigo, nome, telefone, endereco, dataDeNascimento))
        print('\n')
    except Exception as erro:
        print(erro)
